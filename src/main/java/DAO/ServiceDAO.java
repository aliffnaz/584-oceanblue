...
public class ServiceDAO {
	...
	public void addService(ServiceBean bean) {
		...
		try {
			...
			ps = con.prepareStatement(
					"INSERT INTO service(serviceID,serviceName,serviceType,servicePrice,serviceStatus)VALUES(?,?,?,?,?)");
	...
	public void updateService(ServiceBean bean) {
		...
		try {
			...
			// 3. create statement
			ps = con.prepareStatement(
					"UPDATE service SET serviceName=?,serviceType=?,servicePrice=?,serviceStatus=? WHERE serviceID=?");
			ps.setString(1, serviceID);
			ps.setString(2, serviceName);
			ps.setString(3, serviceType);
			ps.setDouble(4, servicePrice);
